...
export async function publishPlanEdit(
  ...
  try {
    ...
    const message =
      catalogError instanceof Error ? catalogError.message : "catalog publish failed";
    notes.push(
      `Catalog version/Revolut step failed (${message}). Entitlements & list price in platform_plan_configs were saved.`,
    );
    console.error("[publishPlanEdit] catalog step failed", catalogError);
  }
