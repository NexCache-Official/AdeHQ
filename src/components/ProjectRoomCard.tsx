      ...
      <Card hover className="group relative h-full overflow-hidden p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate text-[15px] font-semibold text-ink group-hover:text-accent-d">
              {room.name}
            </h3>
            <p className="mt-0.5 line-clamp-2 text-xs text-ink-3">{room.description}</p>
          </div>
          {room.unread > 0 && (
        ...
        <div className="mt-4 flex items-center gap-1.5">
          <div className="flex -space-x-2">
            {room.humans.map((h) => (
              <HumanAvatar key={h} name={state.user?.name ?? "You"} size="sm" className="!h-8 !w-8 ring-2 ring-surface" />
            ))}
          ...
          {employees.length > 4 && (
            <span className="text-xs text-ink-3">+{employees.length - 4}</span>
          )}
        </div>

        <div className="mt-4 flex items-center gap-4 border-t border-border pt-3 text-xs text-ink-3">
