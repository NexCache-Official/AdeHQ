...
/** Strip env/flag leakage from any older sessions still stored in the DB. */
function sanitizeSessionCopy(text: string): string {
  const lower = text.toLowerCase();
  if (
    lower.includes("ai_runtime_v2") ||
    lower.includes("runtime v2 is disabled") ||
    lower.includes("runtimedisabled")
  ) {
    return "Autopilot isn't available right now. Try again in a moment, or ask in chat instead.";
  }
  return text.replace(/^Autopilot hit an error:\s*/i, "").trim() || text;
}
...
export function AutonomousSessionPanel({
  ...
  const livePulse =
    ...
    payload?.session.status === "planning" ||
    payload?.session.status === "queued";
  ...
  const poll = useCallback(async () => {
    try {
      ...
      setError("Couldn't refresh this session. You can close and open it again.");
      if (e instanceof Error) console.warn("[AdeHQ autonomy panel]", e.message);
    }
  }, [sessionId]);
  ...
  const stepPct = session.stepBudget
    ? Math.round((session.stepsUsed / session.stepBudget) * 100)
    : 0;
  const summary = session.resultSummary ? sanitizeSessionCopy(session.resultSummary) : null;
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-card">
      ...
      <div
        className={cn(
          "flex items-start justify-between gap-3 border-b border-border bg-gradient-to-r to-transparent px-4 py-3.5",
      ...
      >
        <div className="flex min-w-0 items-start gap-2.5">
          <span
            className={cn(
              "mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-accent-soft text-accent",
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-ink-2">{session.objective}</p>
          </div>
        </div>
        {onClose && (
          <button
      ...
      {summary && !active && (
        <div
          className={cn(
            "border-t border-border px-4 py-3.5 text-sm leading-relaxed",
            session.status === "failed" ? "bg-muted/50 text-ink-2" : "bg-emerald-500/5 text-ink-2",
          )}
        >
          <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-ink-3">
            <Flag className="h-3 w-3" />
            {session.status === "failed" ? "Status" : "Report"}
          </div>
      ...
      {error && (
        <div className="border-t border-border bg-muted/40 px-4 py-2 text-xs text-ink-2">{error}</div>
      )}

      {/* Controls — only while the session can still be steered */}
      {active && (
...
function StepRow({ step, last }: { step: AutonomousSessionStep; last: boolean }) {
  ...
  const detail = step.detail ? sanitizeSessionCopy(step.detail) : null;
  const title =
    step.kind === "error" && step.title === "Blocked" ? "Couldn't finish" : step.title;
      ...
      <div className={cn("min-w-0 flex-1", last ? "pb-1" : "pb-3")}>
        <div className="flex items-center gap-2">
          ...
          {step.toolName && step.kind === "tool_call" && (
            <span className="rounded bg-ink/5 px-1 py-0.5 font-mono text-[10px] text-ink-3">
              {step.toolName}
            </span>
          )}
        </div>
        {detail && (
          <p className="mt-0.5 whitespace-pre-wrap text-[12px] leading-relaxed text-ink-2">{detail}</p>
        )}
      </div>
    </div>
  );
