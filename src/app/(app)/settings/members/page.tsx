      ...
      <Card className="p-6">
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium text-ink">{member.name ?? "Workspace member"}</div>
                  ...
                  {manage && !isOwner ? (
                    <>
                      <select
                        ...
                        value={assignableRoles().includes(member.role as WorkspaceMemberRole) ? member.role : "member"}
                    ...
                    <span className="rounded-md bg-surface px-2 py-1 text-xs capitalize text-ink-2">{member.role}</span>
                  )}
                </div>
              );
            })}
        ...
        {manage ? (
          ...
          <p className="mt-4 rounded-xl bg-muted/60 px-3 py-3 text-sm text-ink-3">
            Only workspace owners and admins can invite people and change roles.
          </p>
        )}

        {invites.length > 0 && (
          <div className="mt-5">
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-[0.08em] text-ink-3">Pending invitations</h3>
            <div className="space-y-2">
              {invites.map((invite) => (
                <div key={invite.id} className="flex items-center gap-3 rounded-xl border border-border-2 bg-muted/40 px-3 py-2.5">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-soft text-accent">
                    <Mail className="h-4 w-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium text-ink">{invite.invited_email}</div>
                    <div className="text-xs text-ink-3">Role: {invite.role}</div>
                  </div>
                  <span className="rounded-md bg-surface px-2 py-1 text-xs capitalize text-ink-2">{invite.status}</span>
                </div>
              ))}
            </div>
          </div>
