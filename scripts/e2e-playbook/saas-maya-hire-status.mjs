/**
 * SaaS Company 1 — ask Maya (workforce manager) about team fit / next hire, conversationally.
 */
import { chromium } from "playwright";
import fs from "fs";
import path from "path";

const OUT = "/tmp/adehq-saas-maya-hire";
...
try {
  ...
  await page.locator("aside").getByText(/^Maya$/i).first().click();
  await page.waitForURL(/\/rooms\//, { timeout: SHELL_MS });
  await waitShell(page);
  await shot(page, "maya");

  const box = page.getByPlaceholder(/Message|Ask|Write/i).first().or(page.locator("textarea").last());
  await box.fill(
    `Maya — quick check-in as CEO. We've got Lane (PM), Casey (SDR), and Jules (support) humming on FlowDesk. Do we have a gap for a research/ops generalist next, or should we stay lean? Keep it practical. (${stamp})`,
  );
  ...
  if (!/hire|lean|Lane|Casey|Jules|research|ops|team/i.test(main)) {
    bug("P1", "Maya reply missing workforce guidance signals");
  } else {
    note("flow", "Maya gave workforce guidance");
  }
} catch (e) {
