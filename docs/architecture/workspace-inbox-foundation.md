...
- **Reserved / blocked local-parts** (`src/lib/inbox/local-part.ts`): brand names (`adehq`, `nexcache`, `maya`, …), system/role inboxes (`admin`, `support`, …), and an abuse/slur blocklist — these show as reserved, not “taken by another workspace”
